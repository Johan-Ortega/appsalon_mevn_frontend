<script setup>
    ...
    const oneClick = () => {
        ...
        if(lastClickTime && currentTime - lastClickTime < 1500){
            return
        } else {
...
<template>
    ...
    <p class="text-white text-lg">A continuación verifica tu informacion y confirma tu cita</p>
    ...
    <div v-else class="grid gap-5">
        ...
        <p class="text-right text-white text-2xl">Total a pagar: 
            <span class="font-bold">{{ formatCurrency(appointments.totalAmount) }}</span>
        </p>
    </div>
    ...
    <div class="space-y-8" v-if="!appointments.noServicesSelected">
        ...
        <div v-if="appointments.isValidReservation" class="flex justify-end">
            <button
                class="w-full md:w-auto bg-blue-500 p-3 rounded-lg uppercase font-bold text-white"
                id="guardar"
                @click="oneClick"
            >
